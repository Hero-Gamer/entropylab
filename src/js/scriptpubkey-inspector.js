...
const decodeSilentPayment = (text) => {
  ...
  try {
    const { words } = bech32mDecode(raw);
    const data = fromWords(words.slice(1));
    if (data.length < 66) return { state: "invalid-silent-payment", label: "Silent payment address (malformed payload)" };
    return { state: "silent-payment", label: "Silent payment address (BIP352)" };
  } catch {
    return { state: "invalid-silent-payment", label: "Silent payment address (decode failed)" };
  }
};

export const inspectAddress = (input, network = "mainnet") => {
  const raw = String(input ?? "").trim();
  if (!raw) return { state: "empty", label: "Address is empty", scriptHex: null };
  const silent = decodeSilentPayment(raw);
  if (silent) return { ...silent, scriptHex: null };
  try {
    const script = descriptorDerive(`addr(${raw})`, network);
    if (!script) return { state: "invalid", label: "Address could not be decoded for this network", scriptHex: null };
    const classification = classifyScript(script);
    return { state: "ok", label: classification.label, scriptHex: bytesToHex(script), type: classification.type };
  } catch (error) {
    return { state: "invalid", label: error instanceof Error ? error.message : String(error), scriptHex: null };
  }
};
...
const makeInspector = () => {
  ...
  const render = () => {
    ...
    let status = "";
    if (result.state === "match") status = `<div class="scriptpubkey-inspector-status" data-state="match">Match — both resolve to the same scriptPubKey bytes.</div>`;
    else if (result.state === "mismatch") status = `<div class="scriptpubkey-inspector-status" data-state="mismatch">Mismatch — scriptPubKey bytes differ.</div>`;
    ...
    else if (addressInput.value.trim() || scriptInput.value.trim()) status = `<div class="scriptpubkey-inspector-status">Provide a comparable address and scriptPubKey to check equality.</div>`;
